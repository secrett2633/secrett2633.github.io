...
`reStructuredText`를 PEPs의 표준 마크업으로 만들면 섹션 구조, 인라인 마크업, 그래픽 및 테이블 지원을 포함하여 훨씬 더 풍부한 표현이 가능해집니다. 여러 PEPs에는 일반 텍스트 문서가 지원할 수 있는 모든 ASCII 그래픽 다이어그램이 있습니다. PEPs가 HTML 형태로 제공되므로, 적절한 다이어그램을 포함하는 기능은 즉시 유용할 것입니다.
