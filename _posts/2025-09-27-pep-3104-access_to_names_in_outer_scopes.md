...
`global`은 기존 키워드로 214번 나타납니다. `global`을 외부 스코프 키워드로 사용하는 영향을 측정하면, 그러한 변경으로 인해 표준 라이브러리에서 깨질 파일은 18개입니다 (함수가 전역 스코프에 해당 변수가 도입되기 전에 변수를 `global`로 선언하기 때문).
