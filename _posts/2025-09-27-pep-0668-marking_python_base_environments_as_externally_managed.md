...
주요 변경사항은 배포판의 `/usr/bin/python3` 환경에서 `EXTERNALLY-MANAGED` 파일을 추가하여 `pip install`을 기본적으로 비활성화하고, 시스템 관리 패키지를 삭제하지 않도록 하는 것입니다. 컨테이너 이미지의 경우, 기본적으로 마커 파일이 존재하더라도 사용자가 패키지를 설치할 수 있도록 해당 파일을 제거하지 않거나 재정의할 수 있습니다.
