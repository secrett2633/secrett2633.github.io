...
CI (Continuous Integration)에서 JIT 관련 파일이 변경될 때 스텐실 파일은 자동으로 유효성이 검사되고 업데이트됩니다. 이 파일에 영향을 미치는 Pull Request가 열리면, 빌드를 빌드하고 테스트하는 `jit.yml` 워크플로우가 평소와 같이 실행됩니다.
