...
- Part 2에서 선별한 Figure/Table만 `[Figure N]`으로 참조할 수 있습니다. 선별하지 않은 것은 참조하지 마세요.
...
- Part 2에서 선별한 Figure/Table만 `[Figure N]` 또는 `[Table N]`으로 참조하세요. 선별하지 않은 것은 번호로 언급하지 마세요.
...
- **Figure/Table 참조 규칙 (필수)**:
  1. Part 2에서 선별한 Figure/Table은 본문에서 **반드시** `[Figure N]` 또는 `[Table N]` 형태로 참조하세요. 선별했는데 본문에서 참조하지 않으면 안 됩니다.
  2. Part 2에서 선별하지 않은 Figure/Table 번호는 본문에서 절대 언급하지 마세요. 수치 결과를 서술할 때는 번호 없이 내용만 기술하세요.
